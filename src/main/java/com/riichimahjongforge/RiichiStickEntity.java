...
public class RiichiStickEntity extends ThrowableProjectile implements ItemSupplier {

    private static final float KNOCKBACK_RADIUS = 3.0f;
    private static final double KNOCKBACK_STRENGTH = 1.2;

    public RiichiStickEntity(EntityType<? extends RiichiStickEntity> type, Level level) {
        super(type, level);
    }

    public RiichiStickEntity(Level level, LivingEntity thrower) {
        super(RiichiMahjongForgeMod.RIICHI_STICK_ENTITY.get(), thrower, level);
    }

    @Override
    public ItemStack getItem() {
        return new ItemStack(RiichiMahjongForgeMod.MAHJONG_RIICHI_STICK.get());
    }
    ...
    private void dropItem() {
        ...
        ItemEntity itemEntity = new ItemEntity(
                this.level(),
                pos.x, pos.y, pos.z,
                new ItemStack(RiichiMahjongForgeMod.MAHJONG_RIICHI_STICK.get()));
